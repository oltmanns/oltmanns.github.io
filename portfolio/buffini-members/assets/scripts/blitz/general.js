...
$(document).ready(function() {
  ...
  // IF corporate/partner banner exists, add 'blitz-partner' class to body
  if ($('#pnlCorpBranding').is(':visible')) {
    $('body').addClass('blitz-partner');
  }  
  ...
  $('#resourcesAudio a').on('click', function() {
    ...
    $('#modalAudioPlayer').html('<audio controls style="width:100%;"><source src="../assets/audio/blitz/launch/'+id+'.mp3" type="audio/mpeg"></audio>');
  ...
  $('input').on('change', function() {
    if ($(".blitz-action-block.is-complete").length == $(".blitz-action-block").length) {
      ...
      $.getScript("../assets/scripts/blitz/confetti.js");
      $('body').append('<canvas id="confetti" class="blitz-confetti"></canvas>');
    }
  });

  $('#modalWeekFinished').on('closed.zf.reveal', function () {
    $('#confetti').remove();
  });
  
  // Hide resource tab if no tab-content exists
  if ( $('#resourcesPrint').children().length === 0 ) {
    $('#tabPrint').remove();
    $( "#tabs li:first-child" ).trigger("click");
  }
  if ( $('#resourcesAudio').children().length === 0 ) {
    $('#tabAudio').remove();
    $( "#tabs li:first-child" ).trigger("click");
  }
  if ( $('#resourcesVideo').children().length === 0 ) {
    $('#tabVideo').remove();
    $( "#tabs li:first-child" ).trigger("click");
  }
  if ( $('#resourcesLinks').children().length === 0 ) {
    $('#tabLinks').remove();
    $( "#tabs li:first-child" ).trigger("click");
  }
