...
function highlightNavOnScroll() {
  ...
  const sections = Array.from(nav.children).map((item) =>
  ...
  function handleScroll() {
    ...
    if (
      ...
      visibleSection = sections.find(
        (section) => section.offsetTop + section.clientHeight > fromTop
      );
    }
    ...
    document
      .querySelector(`a[href="#${visibleSection.id}"]`)
...
function handleSkillNavToggle() {
  ...
  const handleLinkClicks = (e) => {
    ...
    document
      .querySelector(`[data-skill-group-target="${target}"]`)
      .classList.add("is-active");
    document
      .querySelector(`[data-skill-group="${target}"]`)
      .classList.add("is-active");
  };
